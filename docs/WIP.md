...
0. ✅ **2026-07-23 ARTWORK MODAL round + inline currency convert. All
   ...
   ⓑ **To-Do pill → inline tail-bubble** (`TailBubble`, reuses the fiat-picker
   body-portal + `--p3d-tail-dx` placement; drop-shadow so bubble+tail are one
   seamless piece). Title "Create To-Do for *<Piece #id>*" (piece italic, wraps).
   Verb pick BUY/OFFER/LIST/SEND. Coloured Attention yellow, flips to Hothurt on
   the yellow colorway (bg-is-yellow sense). Bottom row = the TodosBox composer's
   own due(date+time) + P! priority chips, reused verbatim — the picked verb
   files the to-do with them (`addOutputTodo` now takes a priority; set/on accent
   flips to dark ink on the red bubble). Opts out of scroll-dismiss for the
   native pickers.
   ⓒ **Showcase-full "Replace?" bubble** — same tail-bubble treatment, 6 picks as
