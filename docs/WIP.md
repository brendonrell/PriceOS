...
⛔ **BEFORE THE NEXT SEPOLIA RUN — one new setup step, from 2026-08-04.** The
keychain machine now reads a **separate `attestationKey`** off PDFactory (it
used to share `settlementKey`). Nothing on the /deploy page changes — key
setup was never part of the 10 taps — but until that key is proposed and
accepted, **every crank lands at luck 0 and no streak buys better odds.** The
machine still runs; the bonus is just off. Needs a NEW wallet (not the
settlement one, or the split is pointless) with its private key in Cloudflare
alongside `SETTLEMENT_KEY`, then propose from admin + accept from that wallet.
...
review of all 10 contracts, the deployed `$PRICE` token, and the off-chain
keys that hold on-chain authority. Full write-up:
`pd-contracts/audit/2026-08-04-full-stack-pre-mainnet-review.md`.

- **`$PRICE` verified byte-exact** against mainnet — locally compiled runtime
  hash matches the live code exactly. No owner, mint, burn, pause or upgrade;
  supply/name/symbol/decimals all correct; airdrop list matches its published
  SHA-256 (3,005 addresses). Nothing to fix, ever — it's immutable.
- **M-1 sticker supply cap (real bug, PoC'd).** A FIXED sheet reserves supply
  at PURCHASE but delivers the pool at PEEL, so restocking mid-flight handed
  every outstanding sealed copy a sticker nothing was reserved for — proven to
  push a maxSupply-10 sticker to 100 in circulation with `minted` reading 0.
  `addSticker` now freezes a FIXED pool while sealed copies are out;
  `setSheetPull` freezes the mode entirely (the second door into the same
  bug). ⛔ PACK restock deliberately untouched — it draws at peel and checks
  supply there.
- **M-2 drop seats (app).** The seat count a wallet declared was never checked
  against the order it signed, yet it set draw weight AND the on-chain mint
  allowance — enter with a 1-token order, claim 10, take 10× the share. Seats
  are now derived from the signed order (`lib/drops/order.ts`): right chain,
  right project, signed by the session's own wallet, calling `mint(quantity)`.
- **L-1 wallet rotation.** Accept only proved a key could TRANSACT while mints
  PUSH ether — a contract with no `receive()` could take the role and halt all
  primary sales. Accept is now payable and bounces the accepter's own wei back
  through the identical push path. ⛔ Stronger than the zero-value probe the
  audit first proposed, which a non-payable fallback would have passed.
- **L-3 key split** (see the ⛔ setup note above) · **L-4** crank attestation
  bounded at 15 minutes (it carries no nonce by design, so its deadline was
  the only thing stopping one signature being a permanent luck pass) ·
  **L-5** Keychains got the dust rescue PDStickers already had.
- **Deploy artifacts refreshed** — /deploy ships the compiled contracts
  inline and was still carrying pre-fix builds; three changed, the other five
  regenerated byte-identical.
- **L-2 still open and it's a launch action, not code:** the keychain renderer
  is swappable until `lockRenderer()` is called, so until then the art of an
  already-sold charm can be changed. Default is unlocked.
- ⛔ **Two new md rules from this session** (Rule #-0.96 write to be
  understood not to look smart; Rule #-0.95 answer A/B/C, no run-on), plus a
  permanent ban on ever raising outside audit firms — settled, never mention.
