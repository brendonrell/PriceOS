...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data, error } = await sb
      .from('outputs')
      .select('project_id, token_id, dominant_color, aspect, brightness, saturation, complexity, accent_color, palette_count, contrast, warmth, gravity, symmetry, air, texture, scene, shape_count, pattern, geometry, rarity, artist, project_name, true_name, price_day, natal_sun, natal_moon, natal_rising, fate, minted_at')
      .in('project_id', slugs)
