// /api/user/rival — declare (POST) or renounce (DELETE) YOUR rival: the
...
export const POST = requireAuth(async (req, _ctx, address) => {
  ...
  if (target === address.toLowerCase()) return badRequest('You cannot be your own rival');
  try {
    const db = getSupabaseService();
    const up = await db
      .from('users')
      .update({ rival_address: target } as never)
      .eq('address', address.toLowerCase());
    if (up.error) return serverError(up.error.message);
    return NextResponse.json({ ok: true, rival_address: target });
  } catch (err) {
...
export const DELETE = requireAuth(async (_req, _ctx, address) => {
  try {
    ...
    const up = await db
      .from('users')
      .update({ rival_address: null } as never)
      .eq('address', address.toLowerCase());
    if (up.error) return serverError(up.error.message);
    return NextResponse.json({ ok: true, rival_address: null });
  } catch (err) {
    return serverError(err instanceof Error ? err.message : 'Unknown error');
  }
});
