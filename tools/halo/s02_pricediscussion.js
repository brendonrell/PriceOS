/* PRICE DISCUSSION — the founding artifact, the #price-discussion room.
 ...
 *
 * SURREAL = real-but-off (undercurrent): bubbles half-formed, evaporating
 * upward like steam, gravity disobeyed, a familiar chat made atmospheric.
 * Signature palette world: ATTENTION YELLOW on ink — owns yellow-on-dark.
 *
...
window.ENGINE = (function () {
  ...
  /* Six palettes, all in the yellow-on-ink world but each clearly its own
     room/hour so the SET reads varied, not one image reskinned.
       ground  = backdrop ink/paper
       bub     = primary bubble fill (the mass)
       bub2    = secondary bubble fill (the other column / variation)
       accent  = the brand hit (yellow / stamp)
  ...
  function traits(seed) {
    ...
    return { Palette: pal.name, Mode: mode, Format: f, Weather: weatherMap[mode] };
  }

  return { name: 's02_pricediscussion', draw, traits };
})();
