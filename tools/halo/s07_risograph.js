 ...
 * Each ink is its own screen (halftone DOTS, fine LINES, stochastic GRAIN, or a
 * flat SOLID flood) at a distinct angle; overlaps MULTIPLY into a darker mixed
 * third (and, with 3 inks, fourth) colour. Bold simple geometry — arcs, blobs,
 * bars, circles, concentric rings, columns, a fragment of huge type — with
 * confident negative space. SURREAL = real-but-off: the registration drift
 * makes a form GHOST / double or land where it shouldn't; the layers disagree.
 * Restrained, intentional, tactile — NOT messy, NOT clean vector.
 *
 * v2 (2026-06-28 — CEO "go nuts with variations first"): MORE duotone +
 * TRITONE palettes, a per-ink SCREEN treatment axis (Dot / Line / Grain /
 * Solid), and MORE compositional MODES (now incl. Concentric, Columns, Grid,
 * Diagonal) — same clean riso look, far wider gamut of results.
 *
...
window.ENGINE = (function () {
  ...
  /* Riso spot-ink worlds: warm paper stock + 2 OR 3 named inks. Authentic
     slightly-chalky riso pigments. Overprint of any two multiplies darker;
     tritone worlds overprint into rich secondary + tertiary mixes. */
  const PALS = [
    // ── duotone ──
    ...
    { name: 'Cobalt',  paper: '#ece6d6', inks: [
      { name: 'Fluoro Green', hex: '#5fc94a' }, { name: 'Cobalt', hex: '#243f9e' } ] },
    { name: 'Ember',   paper: '#f4ecdb', inks: [
      { name: 'Crimson', hex: '#d6263b' }, { name: 'Charcoal', hex: '#24262b' } ] },
    { name: 'Lagoon',  paper: '#eef0e2', inks: [
      { name: 'Aqua', hex: '#16b3c2' }, { name: 'Magenta', hex: '#d23a8f' } ] },
    // ── tritone ──
    { name: 'Carnival', paper: '#f4ead2', inks: [
      { name: 'Fluoro Pink', hex: '#ff48a0' }, { name: 'Yellow', hex: '#f4c20d' }, { name: 'Blue', hex: '#2046b0' } ] },
    { name: 'Press',    paper: '#f0e9d6', inks: [
      { name: 'Red', hex: '#cf3b2e' }, { name: 'Teal', hex: '#1c8a86' }, { name: 'Black', hex: '#1a1a1a' } ] },
    { name: 'Bazaar',   paper: '#f2ecd8', inks: [
      { name: 'Orange', hex: '#f26b21' }, { name: 'Purple', hex: '#5b2a86' }, { name: 'Sky', hex: '#4aa7d6' } ] },
    { name: 'Grove',    paper: '#eee7cf', inks: [
      { name: 'Green', hex: '#2f8f4e' }, { name: 'Gold', hex: '#e0a51f' }, { name: 'Oxblood', hex: '#7a2222' } ] },
  ];

  const MODES = ['Overprint', 'Drift', 'Halftone', 'Cutout', 'Poster', 'Streak',
                 'Concentric', 'Columns', 'Grid', 'Diagonal'];
  // per-ink screen treatments (riso prints all of these)
  ...
  // cheap deterministic hash → 0..1 (for screen jitter; not the seeded RNG path)
  function fastHash(n) { n = (n ^ 61) ^ (n >>> 16); n = n + (n << 3); n = n ^ (n >>> 4); n = Math.imul(n, 0x27d4eb2d); n = n ^ (n >>> 15); return ((n >>> 0) % 1000) / 1000; }

  /* ── Screen an ink LAYER ──────────────────────────────────────────────────
     Read a WHITE-on-black coverage mask, then render the ink through one of four
     screen treatments rotated to the ink's screen angle, composited later with
     MULTIPLY so overlaps darken into mixed hues.
       Dot   : classic halftone dots, radius ∝ sqrt(coverage)
       Line  : ruling — line thickness ∝ coverage (engraving / screenprint ruling)
       Grain : stochastic dither — random specks with probability ∝ coverage
       Solid : flat ink flood wherever coverage is high (clean spot-colour) */
  function screenLayer(W, H, inkHex, angleDeg, cell, maskCtx, screen) {
    ...
    if (screen === 'Solid') {
      // flat flood: paint the masked area directly, soft edge bleed
    ...
    if (screen === 'Line') {
      // rotated ruling: draw lines across, modulate thickness by coverage along it
      ...
      for (let gy = -diag; gy <= diag; gy += cell) {
        ...
        for (let gx = -diag; gx <= diag; gx += Math.max(2, cell >> 1)) {
          ...
          if (ix < 0 || ix >= W || iy < 0 || iy >= H) { started = false; continue; }
          const cov = sampleCov(ix, iy);
          if (cov < 0.05) { started = false; continue; }
          const th = Math.max(0.5, maxTh * cov);
          // emit a short capsule segment with this thickness
    ...
    if (screen === 'Grain') {
      // stochastic dither: speck density ∝ coverage
  ...
  /* Draw shapes for one ink onto a fresh WHITE-on-black mask canvas, then
     return the screened, registered, ink-coloured layer ready to multiply. */
  ...
  function draw(cv, seed) {
    ...
    // per-ink screen treatments. One coherent treatment most seeds; sometimes
    // mixed (e.g. a solid flood under a dot screen) for richer overprint.
    ...
    // screen angles — classic offset between separations (15° apart triad)
    const baseAng = r() * 30;
    ...
    // ── helper geometry painters (mask space, fill white) ──
    ...
    // ── PER-MODE composition: define what EACH ink paints (array length nInks) ──
    const cxC = W * (0.5 + (r() - 0.5) * 0.16), cyC = H * (0.5 + (r() - 0.5) * 0.16);
    let paints = []; // paints[i] = painter for ink i
    ...
    if (mode === 'Overprint') {
      ...
      // nested rings, each ink owns alternating rings → moiré-free overprint halo
      ...
      // vertical (or horizontal) bars marching across — each ink takes a phase
      const horiz = r() < 0.4;
      ...
      // a contact-sheet of repeated marks; inks offset their grid → ghosting
      const cols = K.rint(r, 3, 5), rows = K.rint(r, 3, 5);
      const mg = S * 0.1;
      const cw = (W - mg * 2) / cols, ch = (H - mg * 2) / rows;
      const mark = K.rint(r, 0, 2); // 0 disc 1 ring 2 bar
      for (let i = 0; i < nInks; i++) {
    ...
    } else { // Diagonal — bold diagonal stripe bands, inks at complementary slopes
    ...
    // pad paints to nInks if a mode produced fewer (safety)
  ...
  function traits(seed) {
    ...
    // mirror draw() rng order: pal, mode, fmt, cell, mixScreens, baseScreen, inkScreens...
    r(); // cell
    const mixScreens = r() < 0.45;
    const baseScreen = K.pick(SCREENS, r);
    const inkScreens = [];
    for (let i = 0; i < pal.inks.length; i++) inkScreens.push(mixScreens ? K.pick(SCREENS, r) : baseScreen);
    const screen = mixScreens ? 'Mixed' : baseScreen;
    return { Palette: pal.name, Mode: mode, Format: f, Screen: screen, Inks: pal.inks.map((i) => i.name).join(' + ') };
  }
