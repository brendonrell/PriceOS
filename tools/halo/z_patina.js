...
window.ENGINE = (function () {
  ...
  function pickAnchor(r) {
    if (window.FORCE_PAL) { const a = ANCHORS.find((p) => p.name === window.FORCE_PAL); if (a) return a; }
  ...
  function traits(seed) {
    ...
    const anchor = window.FORCE_PAL ? (ANCHORS.find((p) => p.name === window.FORCE_PAL) || ANCHORS[0]) : K.pick(ANCHORS, r);
