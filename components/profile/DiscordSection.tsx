 ...
 *
 * Discord is the centre of the PD world, so it gets its own home. Account
 * ASSOCIATION only (not login-with-Discord) — the wallet stays the identity.
 *
 * Two states:
 *   • CONNECTED — the linked identity with personality: real Discord pfp (streamed
 *     from Discord's CDN off a stored hash, no image storage), accent-colour ring,
 *     handle, and a "In the PD Discord" membership badge.
 *   • NOT CONNECTED — clean + tasteful: the purple Discord buttons only (Connect
 *     on your own profile, Join the PD Discord for everyone).
 *
 * Renders for everyone, logged in or not.
 */

import { DISCORD_URL } from '../../lib/config/discord';

/** Discord's brand mark — inline vector (no image asset), inherits button text
 *  colour. */
function DiscordLogo({ size = 20 }: { size?: number }) {
    return (
        <svg className="pd-discord-logo" width={size} height={size} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
