...
import type { RivalRead } from '../../app/api/user/[address]/counterparties/route';
...
export function TopBarRow() {
    ...
    /* Rival HUD (Brendon, 2026-07-20) — the you-vs-rival floor-value delta.
       Doors live in the profile Counterparties tab (summon + dismiss); the
       pill only exists while notifs.rivalHud is on AND a rival is declared.
       ...
       (pd:rival-changed). */
    const [rivalHud, setNemHud] = useState<RivalRead | null>(null);
    useEffect(() => {
        if (!notifs.rivalHud || !siweAddress) { setNemHud(null); return; }
        let cancel = false;
        const load = () => {
            fetch(`/api/user/${siweAddress.toLowerCase()}/counterparties`)
                .then((r) => (r.ok ? r.json() : null))
                .then((j) => { if (!cancel) setNemHud(j?.rival ?? null); })
                .catch(() => { /* keep last read */ });
        };
        load();
        window.addEventListener('pd:rival-changed', load);
        return () => { cancel = true; window.removeEventListener('pd:rival-changed', load); };
    }, [notifs.rivalHud, siweAddress]);
    ...
    /* Rival HUD masks with the grails: Incognito/Hammer hide YOUR portfolio
       read for the same privacy/focus reasons. */
    const hudVisible =
        !!notifs.rivalHud && !!rivalHud && !incognitoActive && !hasHammer;
        ...
        <div className="top-bar" id="topBar">
            ...
            {showRow ? (
                <div className="top-bar-row" id="topBarInner">
                    {/* Rival HUD pill — rides the grail-pill anatomy (Rule #0).
                    ...
                    {hudVisible && rivalHud && (() => {
                        const rival = rivalHud.handle ? `@${rivalHud.handle}` : shortAddress(rivalHud.address);
                        const ahead = rivalHud.mine.floor_value_eth - rivalHud.theirs.floor_value_eth;
                        const verdict = `${ahead >= 0 ? 'AHEAD' : 'BEHIND'} ◊${formatEth(Math.abs(ahead))}`;
                        const go = () => router.push('/' + (myHandle ?? siweAddress ?? ''));
                        return (
                            <span
                                className="grail-pill riv-hud-pill"
                                title={`Rival ${rival} — you ◊${formatEth(rivalHud.mine.floor_value_eth)} · them ◊${formatEth(rivalHud.theirs.floor_value_eth)}, at today’s floors`}
                                onClick={go}
                                role="button"
                                tabIndex={0}
