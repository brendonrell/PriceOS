...
/*
 * priceSpriteEngine — animation state machine for the main sprite.
 *
 ...
 *
 * Action animations (layered on top):
 ...
 *
 * armVariant (0|1): random per event → two poses per action animation.
 *
 * Turn transform rules:
 *   glyph-swap (_mirrorMode=false, facing=-1): arm chars change, no CSS flip
 *   css-mirror (_mirrorMode=true,  facing=-1): arm stable, scaleX(-1)
...
let _argueTimer: ReturnType<typeof setTimeout> | null = null;
let _throwTimer: ReturnType<typeof setTimeout> | null = null;
    ...
    if (_identity !== null) {
        ...
        if (composed !== null) {
            ...
            return {
                ...
                shadesLens:  composed.shadesLens,
            };
        }
    }

    // Standin path
    const standinLeft = isLeft || isThrowing;
    let face: string;
    if      (_state === 'sleeping')                         face = STANDIN_SLEEP_R;
    else if (_state === 'yawning')                          face = STANDIN_YAWN_R;
    else if (_state === 'blinking')                         face = (_mirrorMode || !standinLeft) ? STANDIN_BLINK_R : STANDIN_BLINK_L;
    else if (_state === 'arguing' || _state === 'casting')  face = STANDIN_AWAKE_R;
    else                                                    face = (_mirrorMode || !standinLeft) ? STANDIN_AWAKE_R : STANDIN_AWAKE_L;
    const transform = (_mirrorMode && isLeft && !isAsleep) || isThrowing
        ? 'scaleX(-1)' : 'scaleX(1)';
...
function _clearAll(): void {
    [_idleTimer, _blinkTimer, _turnTimer, _stateTimer,
     _argueTimer, _throwTimer, _castTimer]
        .forEach(t => { if (t) clearTimeout(t); });
    _idleTimer = _blinkTimer = _turnTimer = _stateTimer =
    _argueTimer = _throwTimer = _castTimer = null;
}
...
function _blink(): void {
    ...
    _stateTimer = setTimeout(() => { if (_state === 'blinking') { _state = 'awake'; _emit(); } }, 160);
}
...
function _scheduleThrow(): void {
    if (_throwTimer) clearTimeout(_throwTimer);
    _throwTimer = setTimeout(() => {
...
function _scheduleArgue(): void {
    if (_argueTimer) clearTimeout(_argueTimer);
    _argueTimer = setTimeout(() => {
