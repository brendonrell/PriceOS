// Social activity feed ☻ builder — "what your friends are doing, in
// chronological order" (Brendon, 2026-07-26). Logged in: ledger events ACTED
// by the viewer's social graph, with mutuals outranking one-way follows
// whenever the window overflows the page. Logged out (or an empty graph): the
// top collectors by PriceScore, so the feed reads as the platform's social
// pulse out of the box. No user options, by design — one optimized feed for
// everyone.
//
...
//
// Lives here (not in app/api/feed/social/route.ts) so the home page's server
// render can call the builders directly to seed SocialFeed (Brendon,
// 2026-08-26). Next.js's typed routes only allow route.ts to export HTTP
// method handlers, so a plain helper export there fails the build's
// route-shape check — that's what broke the Cloudflare build.
...
export const GRAPH_CAP = 120;
export const WINDOW = 200;
export const TOP_USERS = 50;
...
export function toEventRow(e: DbEvent): EventRow | null {
  let type: EventType;
...
export function actorAddr(e: EventRow): string | null {
  const a = e.type === 'MINT' || e.type === 'SALE' ? e.to_address : e.from_address;
  return a ? a.toLowerCase() : null;
}

export type Db = ReturnType<typeof getSupabaseService>;
...
export async function graphAddresses(
  db: Db,
  viewer: string,
...
export async function topAddresses(db: Db): Promise<Set<string>> {
...
export async function albumsFor(
  db: Db,
  addrs: string[],
...
export async function eventsFor(db: Db, addrs: string[]): Promise<EventRow[]> {
...
/* Anonymous default page — the top-collectors fallback, extracted so the
   ...
   logged-out/empty-graph branch calls this too. */
