...
export interface UpdateFaviconArgs {
    bg: string | null;
    fg: string | null;
    text: string;
    isAlert?: boolean;
    ...
    /* The viewed profile's Profile Logo. When set, the favicon paints THIS in
       place of the colorway logo, matching the corner logo (Brendon, 2026-06-23). */
    profileLogo?: ProfileLogoFavicon | null;
}

const HOLO_STOPS: [number, string][] = [
    [0, '#FF6EC4'], [0.22, '#7873F5'], [0.44, '#4ADEDE'],
    [0.66, '#9EFF6E'], [0.84, '#FFE86E'], [1, '#FF9F6E'],
];
...
export function updateFavicon({
    ...
    ctx.save();
    ctx.translate(32, 32);
    if (isRotated) ctx.rotate((-90 * Math.PI) / 180);
    ctx.translate(-32, -32);

    if (isEthPing) {
        ctx.fillStyle = '#FF0033'; // Hothurt Red
        ctx.font = "bold 50px 'Courier New', monospace";
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText('\u27E0', 32, 36);
    } else if (profileLogo && profileLogo.kind === 'price') {
        // Profile $PRICE logo \u2014 the wordmark recoloured to the owner's pick.
        const plate = profileLogo.bg ?? '#FF0055';
        const letters = profileLogo.fg ?? '#FFE600';
        const svg = PRICE_LOGO_SVG
            .replace(/#FF0055/g, '#PLPLATE_')
            .replace(/#FFE600/g, letters)
            .replace(/#PLPLATE_/g, plate);
        const img = new Image();
        img.onload = () => {
            ctx.clearRect(0, 0, 64, 64);
            const fh = Math.round((64 * 403) / 517);
            ctx.drawImage(img, 0, Math.round((64 - fh) / 2), 64, fh);
            ctx.restore();
        ...
        // Profile bubble logo \u2014 mirror the corner logo's colours + finish.
        const color = profileLogo.color ?? '#FF0055';
        const cutout = profileLogo.cutout ?? '#FFE600';
        ctx.save();
        ctx.scale(0.084, 0.084);
        ctx.translate(0, 50);
        const bubblePath = new Path2D(BUBBLE_PATH);
        let bubbleFill: string | CanvasGradient = color;
        if (profileLogo.holo) {
            const grad = ctx.createLinearGradient(0, 0, 761, 655);
            for (const [stop, c] of HOLO_STOPS) grad.addColorStop(stop, c);
            bubbleFill = grad;
        }
        if (profileLogo.glyphOnly) {
            // Per-mille only \u2014 no bubble, the mark painted solid in the colour.
            ctx.fillStyle = color;
            ctx.fill(new Path2D(INNER_SYMBOL_PATH));
        } else if (profileLogo.outline) {
            // Hollow bubble (stroke) + the mark in the same colour.
            ...
            ctx.fill(bubblePath);
            // Blank = bubble only; otherwise knock the \u2030 in the cutout colour and
            // carve the dot centres back to the bubble fill (matches the corner).
            if (!profileLogo.blank) {
                ctx.fillStyle = cutout;
                ctx.fill(new Path2D(INNER_SYMBOL_PATH));
                ctx.fillStyle = bubbleFill;
                ctx.fill(new Path2D(HOLE1_PATH));
                ctx.fill(new Path2D(HOLE2_PATH));
                ctx.fill(new Path2D(HOLE3_PATH));
            }
        }
        ctx.restore();
    } else if (priceLogoOverride) {
        // Async path — SVG decode → drawImage → toDataURL on load.
        const img = new Image();
        img.onload = () => {
            ctx.clearRect(0, 0, 64, 64);
            // Sim 5587: SVG is 517x403 — fit full width (64), center vertically.
            const fh = Math.round((64 * 403) / 517);
            ctx.drawImage(img, 0, Math.round((64 - fh) / 2), 64, fh);
            ctx.restore();
            const link = document.getElementById(FAVICON_LINK_ID) as HTMLLinkElement | null;
        ...
        // Default mark: vector ‰ wordmark inside the bubble.
        if (text === '\u2030' && !isAlert) {
            ctx.fillStyle = fg ?? '#FFFFFF';
            const innerSymbol = new Path2D(INNER_SYMBOL_PATH);
            ctx.fill(innerSymbol);
            ctx.fillStyle = bg ?? '#000000';
            const hole1 = new Path2D(HOLE1_PATH);
            const hole2 = new Path2D(HOLE2_PATH);
            const hole3 = new Path2D(HOLE3_PATH);
            ctx.fill(hole1);
            ctx.fill(hole2);
            ctx.fill(hole3);
        }
        ...
        // Text-override mode: any text other than the default ‰, or alert state.
        if (text !== '\u2030' || isAlert) {
            ctx.fillStyle = isAlert ? '#FFFFFF' : (fg ?? '#FFFFFF');
            ctx.font = 'bold 26px Arial, sans-serif';
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
            ctx.fillText(text, 34, 30);
        }
    }

    ctx.restore();

    // Sync path: write the data URL to the link tag. The async branches (the
    // $PRICE wordmark + a profile $PRICE logo) return early and write in onload.
    const link = document.getElementById(FAVICON_LINK_ID) as HTMLLinkElement | null;
    if (link) {
