// @ts-nocheck
/* NOISE FROM BELOW 2 — abstract geology, core samples made of WRONG matter.
 ...
 *
 * The sequel "Noise From Below 2" — straight production port of the R&D engine
 * tools/halo/s10_strata.js. Art unchanged; window.KIT/window.ENGINE/
 * window.FORCE_PAL removed; palette by rng only. Deterministic in tokenId.
 */
...
export const below2Traits: TraitsFn = (seed) => {
  const r = K.rng(seed);
  const pal = K.pick(PALS, r);
  const mode = K.pick(MODES, r);
  let fmt;
...
export const below2Schema: TraitSchema = {
  traits: [
...
export const renderBelow2: EngineFn = K.blit((off, id) => { draw(off, id); }, below2Traits);

// W/H of the FORMATS entries (Portrait P, Square S, Landscape L).
export const BELOW2_ASPECTS = [1040 / 1320, 1, 1340 / 1000] as const;
