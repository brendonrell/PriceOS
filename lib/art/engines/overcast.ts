// @ts-nocheck
/*
 * Overcast — PriceOS art engine (ported from the halo R&D engine
 * tools/halo/h4_overcast.js). Continuous seed-driven surreal "real-but-off"
...
export const overcastTraits: TraitsFn = (tokenId) => ENGINE.traits(tokenId);
export const renderOvercast: EngineFn = blit((cv, tokenId) => ENGINE.draw(cv, tokenId), (tokenId) => ENGINE.traits(tokenId));
export const OVERCAST_ASPECTS: readonly number[] = [1.5006,1];
export const overcastSchema: TraitSchema = {
  "traits": [
    {
      "name": "Palette",
      "values": [
