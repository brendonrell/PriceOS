// @ts-nocheck
/*
 * Topiary — PriceOS art engine (ported from the halo R&D engine
 * tools/halo/gen/topiary.js). Layout axis (4-6 structurally distinct
...
export const topiaryTraits: TraitsFn = (tokenId) => ENGINE.traits(tokenId);
export const renderTopiary: EngineFn = blit((cv, tokenId) => ENGINE.draw(cv, tokenId), (tokenId) => ENGINE.traits(tokenId));
export const TOPIARY_ASPECTS: readonly number[] = [1.3214,1,0.831];
export const topiarySchema: TraitSchema = {
  "traits": [
    {
      "name": "Layout",
      "values": [
