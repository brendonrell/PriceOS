// @ts-nocheck
/*
 * Ballast — PriceOS art engine (ported from the halo R&D engine
 * tools/halo/h4_ballast.js). Continuous seed-driven surreal "real-but-off"
...
export const ballastTraits: TraitsFn = (tokenId) => ENGINE.traits(tokenId);
export const renderBallast: EngineFn = blit((cv, tokenId) => ENGINE.draw(cv, tokenId), (tokenId) => ENGINE.traits(tokenId));
export const BALLAST_ASPECTS: readonly number[] = [0.8,1.5];
export const ballastSchema: TraitSchema = {
  "traits": [
    {
      "name": "Palette",
      "values": [
