...
function grain(x: Ctx, W: number, H: number, amt: number, r: R) { const n = Math.floor(W * H / amt); for (let i = 0; i < n; i++) { const g = r() < 0.5 ? 0 : 255; x.fillStyle = `rgba(${g},${g},${g},${0.012 + r() * 0.04})`; x.fillRect(r() * W, r() * H, 1, 1); } }
