// @ts-nocheck
/*
 * Aperture — PriceOS art engine (ported from the halo R&D engine
 * tools/halo/b_aperture.js). Continuous seed-driven composition. Deterministic
...
const ENGINE = (function () {
  ...
  function traits(seed) {
    ...
    const world = undefined ? (PALS.find((p) => p.name === undefined) || PALS[0]) : K.pick(PALS, r);
...
export const apertureTraits: TraitsFn = (tokenId) => ENGINE.traits(tokenId);
export const renderAperture: EngineFn = blit((cv, tokenId) => ENGINE.draw(cv, tokenId), (tokenId) => ENGINE.traits(tokenId));
export const APERTURE_ASPECTS: readonly number[] = [1.3,1,0.78];
export const apertureSchema: TraitSchema = {
  "traits": [
    {
      "name": "Palette",
      "values": [
