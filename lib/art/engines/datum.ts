// @ts-nocheck
/*
 * DATUM — engineering drawings of IMPOSSIBLE OBJECTS on blueprint / vellum.
 *
 * Rebuilt (2026-06-28) from the R&D engine s11_vellum into a real GENERATIVE
 * SYSTEM, per the CEO note: "gen art is building a system you introduce
 * randomness to; this isn't mixing-and-matching modes." The old engine had six
 * fixed modes (Plan/Elevation/Exploded/...) — one template recoloured. DATUM
 * replaces that with a PARAMETRIC drafting system:
 *
 *   • The impossible object ITSELF is procedurally generated — a chain of
 *     N beams (2–4) assembled into a loop that cannot close in 3-space (a
 *     generalised Penrose polygon / Escher frame), with a per-seed beam width,
 *     turn handedness, and corner count.
 *   • PROJECTION is a continuous parameter: an isometric/axonometric basis whose
 *     azimuth + elevation are drawn per seed (orthographic flattens elevation
 *     toward 0). No two seeds share a viewing angle.
 *   • LAYOUT is generated, not chosen: a sheet carries 1–4 VIEWS placed on a
 *     procedural grid (single hero, stacked pair, triptych, quad multiview),
 *     each view a different projection/zoom of the SAME generated object so the
 *     orthographic views genuinely disagree (the surreal core).
 *   • Every annotation layer is parametric & independently dialled: dimension-
 *     line density, leader-callout count, section hatching density, an optional
 *     exploded offset that flies the beams apart along an assembly axis, sheet
 *     count + skew for layered vellum, and procedural title-block content.
 *
 * Across seeds the OUTPUT is genuinely diverse: different impossible objects,
 * different projections, different view counts and layouts — not one drawing
 * recoloured. The 6 palettes + the drafting craft (witness lines, arrowheads,
 * drafting numerals, 45° hatch, title block, vellum grade) are preserved.
 *
 * Deterministic from tokenId only. ALL trait-determining randomness is drawn in
 * paramsOf() in a FIXED order so datumTraits() and draw() never disagree.
 */
import { rng, pick, rint, randn, clamp, mix, rgba, makeNoise, hazeSheet, mottle, grain, vignette, blit } from './ai/extra/_kit';
import type { EngineFn, TraitsFn, TraitSchema } from '../../project/types';

const K = { rng, pick, rint, randn, clamp, mix, rgba, makeNoise, hazeSheet, mottle, grain, vignette };

/* The 6 drafting palettes — preserved verbatim from the R&D engine.
   ground + line ink(s); `dim` = dimension/ferro accent; `note` = annotation. */
const PALS = [
...
/* The PARAMETRIC axes that become traits. Each is a genuine system knob. */
const LAYOUTS  = ['Hero', 'Stacked Pair', 'Triptych', 'Quad Multiview'];
const PROJECTIONS = ['Isometric', 'Axonometric', 'Orthographic', 'Oblique'];
// Generated object families (driven by corner count + handedness, named for the trait).
const OBJECTS  = ['Tri-Loop', 'Quad-Frame', 'Penta-Brace', 'Twist Lattice'];
const DENSITY  = ['Sparse', 'Measured', 'Annotated', 'Overspecified']; // annotation density tier

/* drafting numerals + notes (preserved) */
...
/* ── PARAM DRAW (FIXED ORDER — these become the traits) ───────────────────── */
function paramsOf(r) {
  const palI       = Math.floor(r() * PALS.length);          // 1: palette
  const fmtI       = Math.floor(r() * FORMATS.length);       // 2: format
  const layoutI    = Math.floor(r() * LAYOUTS.length);       // 3: layout (view count)
  const projI      = Math.floor(r() * PROJECTIONS.length);   // 4: projection family
  const corners    = K.rint(r, 3, 5);                        // 5: beams in the impossible loop (3..5)
  const hand       = r() < 0.5 ? 1 : -1;                     // 6: turn handedness
  const beamW      = 0.20 + r() * 0.18;                      // 7: beam width fraction
  const densityI   = Math.floor(r() * DENSITY.length);       // 8: annotation density tier
  const exploded   = r() < 0.32;                             // 9: exploded assembly?
  const sheets     = r() < 0.30 ? K.rint(r, 2, 3) : 1;       // 10: layered vellum sheets
  const grid       = r() < 0.85;                             // 11: engineering grid on?
  // continuous projection angles (derived from projection family + jitter)
  const azBase = projI === 3 ? -0.95 : -Math.PI / 6;         // oblique skews harder
  const az = azBase + K.randn(r) * 0.18;                     // 12
  const elBase = projI === 2 ? 0.06 : projI === 1 ? 0.30 : 0.46; // ortho ~flat, axon mid, iso tall
  const el = elBase + K.randn(r) * 0.06;                     // 13
  return {
    palI, fmtI, layoutI, projI, corners, hand, beamW, densityI,
    exploded, sheets, grid, az, el,
  };
}
...
function draw(cv, seed) {
  ...
  // projection basis vectors (unit-length, scaled per-view by R)
  const ix = [Math.cos(P.az), Math.sin(P.az) * P.el];
  const iy = [Math.cos(Math.PI - P.az), Math.sin(Math.PI - P.az) * P.el];
  const iz = [0, -1];

  // ── BASE GROUND with faint tonal unevenness (preserved grade) ──
  ...
  /* ── line helpers (crisp thin defaults) ── */
  ...
  function solid(p, col, w, a, close) { setLine(col || pal.ink, w || 1.4, a); poly(p, close); x.stroke(); }
  ...
  /* ════════════════════════════════════════════════════════════════════════
   * THE GENERATED IMPOSSIBLE OBJECT
   * A loop of `corners` beams. Walk the corners of a regular polygon; each beam
   * is an extruded bar between consecutive corners. The illusion: every beam's
   * far end is lifted one z-step, so the loop spirals yet is drawn as if closed
   * — a generalised Penrose polygon. Drawn back-to-front for clean overlaps.
   * Returns nothing; paints into the current transform centred on (0,0).
   * ════════════════════════════════════════════════════════════════════════ */
  ...
  function impossibleLoop(cx, cy, R, beamFrac, corners, hand, col, w, fillA, explode) {
    ...
    const bw = u * beamFrac * 2;
    // polygon corner directions in the (a,b) ground plane
    const pts = [];
    for (let i = 0; i < corners; i++) {
      const ang = hand * (i / corners) * Math.PI * 2 - Math.PI / 2;
      pts.push([Math.cos(ang) * u * 1.6, Math.sin(ang) * u * 1.6, i]); // a,b, step
    }
    // build each beam as a 3D box from corner i to corner i+1, lifted by i.
    // z rises by one beam-width per corner → the impossible spiral.
    const beams = [];
    for (let i = 0; i < corners; i++) {
      const A = pts[i], B = pts[(i + 1) % corners];
      const zA = i * beamFrac + (explode ? i * 0.9 : 0);
      const zB = (i + 1) * beamFrac + (explode ? (i + 1) * 0.9 : 0);
      // beam cross direction (perpendicular in ground plane)
      const dx = B[0] - A[0], dy = B[1] - A[1], dl = Math.hypot(dx, dy) || 1;
      const px = -dy / dl * beamFrac * u, py = dx / dl * beamFrac * u;
      // 8 corners of the extruded bar (ground rect × top rect)
      ...
      // painter depth ≈ projected y of the beam centre
      const depth = (c000[1] + c011[1] + t000[1] + t011[1]) / 4;
      beams.push({ depth, faces: [
        [t000, t001, t011, t010],   // top
        [c001, t001, t011, c011],   // outer side
        [c000, t000, t010, c010],   // inner side
        [c010, t010, t011, c011],   // end cap (far)
      ] });
    }
    ...
    for (const beam of beams) {
      beam.faces.forEach((face, fi) => {
        ...
        x.closePath();
        // knock-out fill so overlaps read solid (the illusion needs occlusion)
        x.fillStyle = fi === 0
  ...
  /* one VIEW: framed region, centre lines, the generated object at a per-view
     zoom + projection-jitter, optional dimension chain + leaders + hatch. */
  function drawView(bx, by, bw, bh, label, zoom, allowDims) {
    ...
    // exploded part balloons along an assembly axis
    if (P.exploded) {
    ...
    // ── annotation density tier drives how much measuring chrome appears ──
    const tier = P.densityI; // 0 sparse .. 3 overspecified
    ...
    for (let i = 0; i < ldrN; i++) {
      ...
      const lx = cx + Math.cos(ang) * (R * 1.9 + bw * 0.12) * (Math.cos(ang) < 0 ? 1 : 1);
    ...
    // a section hatch wedge on the densest tier — a "cut" through the object
    if (tier >= 3) {
  ...
  /* ── LAYOUT: place 1–4 views on a procedural grid. Each view gets a slightly
       different zoom so multiviews of the same object visibly disagree. ── */
  const margin = S * 0.07;
  const inner = { x: margin * 1.4, y: margin * 1.6, w: W - margin * 2.8, h: H - margin * 3.2 };

  function placeSheet() {
    if (P.layoutI === 0) {
      // Hero — single large view
      drawView(inner.x, inner.y, inner.w, inner.h, 'GENERAL VIEW', 1.0, true);
    } else if (P.layoutI === 1) {
      // Stacked Pair — two views one above the other, disagreeing
      ...
      // Triptych — two top, one wide bottom
      ...
      // Quad Multiview — 2×2 classic orthographic layout, all disagreeing
  ...
  // layered vellum sheets — draw the whole composition `sheets` times, offset
  ...
  // ── TITLE BLOCK (procedural content) ──
...
/* faint engineering grid behind everything (preserved) */
...
/* corner title block — ruled boxes, procedural part number, SCALE, projection */
...
export const datumTraits: TraitsFn = (id) => labels(paramsOf(rng(id)));

export const datumSchema: TraitSchema = {
  traits: [
    { name: 'Palette',    values: PALS.map((p) => p.name) },
    { name: 'Format',     values: ['Portrait', 'Square', 'Landscape'] },
    { name: 'Layout',     values: LAYOUTS },
    { name: 'Projection', values: PROJECTIONS },
    { name: 'Object',     values: OBJECTS },
    { name: 'Annotation', values: DENSITY },
  ],
};

export const renderDatum: EngineFn = blit(draw, datumTraits);

// W/H of portrait / square / landscape (matches FORMATS order).
export const DATUM_ASPECTS = [1040 / 1300, 1, 1300 / 1000] as const;
