 ...
 *
 * Straight production port of the R&D engine tools/halo/s06_heliodon.js.
...
const lightMap = { 'Equinox': 'Equinox', 'Solstice': 'Solstice', 'Gnomon': 'Greyscale', 'Blue Hour': 'Blue Hour', 'Terracotta': 'Golden', 'Eclipse': 'Eclipse' };

export const heliodonTraits: TraitsFn = (seed) => {
  const r = K.rng(seed);
  const pal = K.pick(PALS, r);
  const mode = K.pick(MODES, r);
  const fmt = K.pick(FORMATS, r);
  const f = fmt[0] > fmt[1] ? 'Landscape' : fmt[0] === fmt[1] ? 'Square' : 'Portrait';
  return { Palette: pal.name, Mode: mode, Format: f, Light: lightMap[pal.name] };
};

export const heliodonSchema: TraitSchema = {
  traits: [
    { name: 'Palette', values: PALS.map((p) => p.name) },
    { name: 'Mode',    values: MODES },
    { name: 'Format',  values: ['Portrait', 'Square', 'Landscape'] },
    { name: 'Light',   values: PALS.map((p) => lightMap[p.name]) },
  ],
};

export const renderHeliodon: EngineFn = K.blit((off, id) => { draw(off, id); }, heliodonTraits);

// W/H of the FORMATS entries (portrait, square, landscape).
export const HELIODON_ASPECTS = [1040 / 1280, 1, 1300 / 1020] as const;
