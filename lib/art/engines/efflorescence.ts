// @ts-nocheck
/*
 * Efflorescence — PriceOS art engine (ported from the halo R&D engine
 * tools/halo/z_efflorescence.js). Continuous seed-driven composition. Deterministic
...
export const efflorescenceTraits: TraitsFn = (tokenId) => ENGINE.traits(tokenId);
export const renderEfflorescence: EngineFn = blit((cv, tokenId) => ENGINE.draw(cv, tokenId), (tokenId) => ENGINE.traits(tokenId));
export const EFFLORESCENCE_ASPECTS: readonly number[] = [1.3,1,0.78];
export const efflorescenceSchema: TraitSchema = {
  "traits": [
    {
      "name": "Palette",
      "values": [
